...
function LongPressStoryPostModal(props) {
	...
	const submitNewStory = async () => {
		...
		axios
			.post("https://api.thearqive.com/api/pins/", pin, config)
			.then((res) => {
			...
			<SafeAreaView style={styles.container} forceInset={{ top: "always" }}>
				...
				>
					...
					>
						...
						>
							<Text style={{ fontSize: 16, color: colors.purple, padding: 24 }}>
								done
							</Text>
						</TouchableOpacity>
					</TouchableWithoutFeedback>
...
const mapStateToProps = (state) => {
	...
	const userName =
		state.authReducer.isLoggedIn === true
